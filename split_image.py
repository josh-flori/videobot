...
# load image
for t in range(46):
    image = cv2.imread('/users/josh.flori/desktop/memes/'+str(t)+'.jpg')
...
#
    ...
    divs=[divs[0][i]+3 for i in range(len(divs[0])) if divs[0][i]>30 and divs[0][i] < image.shape[0]-40] 
    divs=[divs[i+1] for i in range(len(divs)-1) if divs[i+1]-divs[i]>image.shape[0]*.037] 
    # split up image
    for i in range(len(divs)):
        if i==len(divs)-1:
            cv2.imwrite('/users/josh.flori/desktop/'+str(t)+str(i)+'.jpg',image[0:divs[i],])
            cv2.imwrite('/users/josh.flori/desktop/'+str(t)+str(i+1)+'.jpg',image)
        else:
            cv2.imwrite('/users/josh.flori/desktop/'+str(t)+str(i)+'.jpg',image[0:divs[i],])