...
def create_video(image_paths,parameters):
    ...
    clips = [ImageClip(image_paths[i]).set_duration(parameters[i][4]-.4) if i < len(image_paths)-1 else ImageClip(image_paths[i]).set_duration(parameters[i][4]+.5) for i in range(len(image_paths))] # <-- give the last frame a little time to breath before jumping into the next 
